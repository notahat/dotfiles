...
local function map_global_keys()
    ...
    map(nx, "<leader>/", telescope.live_grep, { desc = "live grep" })
    ...
    map(nx, "<leader>h", vim.cmd.nohlsearch, { desc = "hide search highlighting" })
    map(nx, "<leader>n", fidget.notification.show_history, { desc = "show notification history" })
    -- map(nx, "<leader>n", mini_notify.show_history, { desc = "show notification history" })
    map(nx, "<leader>Q", actions.write_all_and_quit, { desc = "save all files and quit" })
    map(nx, "<leader>R", actions.restore_session, { desc = "restore previous session" })
    ...
    which_key.add({ "<leader>c", group = "code changes" })
    ...
    map("i", "<d-v>", '<esc>"*pa', { desc = "paste from system clipboard" })
